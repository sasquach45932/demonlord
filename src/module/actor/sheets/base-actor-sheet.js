...
export default class DLBaseActorSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
  ...
  async _prepareContext(options) {
    ...
    context.iseBaneModeRevised = game.settings.get('demonlord', 'optionalRuleBaneMode') === 'r' ? true : false
    context.isHorrifying = this.actor.items.find(x => x.name.toLowerCase().includes('horrifying') && x.type === 'feature')
    context.isFrightening = this.actor.items.find(x => x.name.toLowerCase().includes('frightening') && x.type === 'feature')
